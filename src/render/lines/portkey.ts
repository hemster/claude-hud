...
function getDailyColor(percent: number): string {
  if (percent >= 125) return EVA_PURPLE;
  if (percent >= 75) return BRIGHT_BLUE;
  return CYAN;
}

function dailyBar(percent: number, width: number): string {
...
function readCache(): PortkeyCache | null {
  try {
    const cachePath = getCachePath();
    if (!fs.existsSync(cachePath)) return null;
    const raw = fs.readFileSync(cachePath, 'utf-8');
    const data = JSON.parse(raw) as PortkeyCache;
...
export function renderPortkeyLine(ctx: RenderContext): string | null {
  ...
  const fmtAvg = (n: number) => n >= 1000000 ? `${(n / 1000000).toFixed(0)}M` : n >= 1000 ? `${(n / 1000).toFixed(0)}k` : `${n}`;
