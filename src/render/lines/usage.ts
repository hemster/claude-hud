...
export function renderUsageLine(
  ...
  if (usageCompact) {
    ...
    const compactParts = [fiveHourPart, sevenDayPart, ...modelParts].filter(
  ...
  const withModelParts = (line: string): string =>
    expandedModelParts.length > 0 ? `${line} | ${expandedModelParts.join(" | ")}` : line;
